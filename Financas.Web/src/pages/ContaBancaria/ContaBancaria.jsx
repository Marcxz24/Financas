...
function ContaBancaria() {
  ...
  useEffect(() => {
    ...
    return () => { ignore = true; }; // Cleanup para evitar atualizações em componentes desmontados
  }, [buscarContas]);
  ...
  const handleSalvar = async (e) => {
    ...
    try {
      if (modoEdicao) {
        await api.patch(`/contas-bancarias/Atualizar-conta-bancaria/${formulario.id}`, payload);
      } else {
  ...
  const handleExcluir = async (id) => {
    if (!window.confirm("Deseja realmente excluir esta conta? Lançamentos vinculados serão removidos.")) return;
  ...
  const prepararEdicao = (conta) => {
    const enumMap = {
      Corrente: 1, Poupanca: 2, Digital: 3, Salario: 4, 
      Pagamento: 5, Investimento: 6, Universitaria: 7, Internacional: 8,
    };
  ...
  const resetarFormulario = () => {
    setFormulario({ id: null, nomeContaBancaria: "", tipoContaBancaria: 1, saldoContaBancaria: 0 });
    setModoEdicao(false);
  };
  ...
  return (
    <div className="contas-container">
      ...
      <header className="contas-header">
        <h1><i className="bi bi-bank"></i> Minhas Contas Bancárias</h1>
        ...
        <form onSubmit={handleSalvar}>
          <div className="form-grid">
            <div className="input-group">
              ...
              <input type="text" name="nomeContaBancaria" value={formulario.nomeContaBancaria || ""} onChange={handleInputChange} placeholder="Ex: Nubank Principal" required />
            </div>

            <div className="input-group">
              <label>Tipo de Conta</label>
              <select name="tipoContaBancaria" value={formulario.tipoContaBancaria || ""} onChange={handleInputChange} required>
                {!modoEdicao && <option value="" disabled>Selecione um tipo...</option>}
            ...
            <div className="input-group">
              ...
              <input type="number" name="saldoContaBancaria" value={formulario.saldoContaBancaria || ""} onChange={handleInputChange} step="0.01" disabled={modoEdicao} placeholder="0,00" required />
              {modoEdicao && <small>O saldo deve ser alterado via lançamentos.</small>}
            </div>
          </div>

          <div className="form-actions">
            <button type="submit" className="btn-salvar">{modoEdicao ? "Salvar Alterações" : "Cadastrar Conta"}</button>
            {modoEdicao && <button type="button" className="btn-cancelar" onClick={resetarFormulario}>Cancelar</button>}
          </div>
        </form>
      </section>
